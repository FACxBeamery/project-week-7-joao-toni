...
const toggleUserTask = (userIdToUpdate, dayOfTask, taskID) => {
    const db = getDb();
    ...
    return new Promise((resolve, reject) => {
        users.findOne(findTaskInUsersQuery, (err, result) => {
            if (err) reject(err);
            try {
                ...
                tasksForDay.some(task => {
                    let doesTaskIdMatchTaskToUpdate =
                ...
                let updateTaskProgressQuery = {
                    $set: {
                        [`tasks.${dayOfTask}.$.progress`]: mapTaskProgressToggles[
                ...
                if (err instanceof TypeError) {
                    ...
                    console.error(err);
                    reject({
                        message: `No user or task found for user id '${userIdToUpdate}'`
                    });
                } else reject(err);
            }
        });
