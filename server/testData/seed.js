...
const addSeedData = async (db) => {
    try {
        ...
        await Promise.all(seedUsersData.map((user) =>
            db.collection("users").insertOne(user)
        ));

        const allUsersResults = await db.collection("users").find({}).toArray();
        ...
        allUsersId.forEach(async (user) => {
            await Promise.all(seed.map(async (task) => {
                const insertTaskResult = await addUserTask(db, task);

                let parsedTask = parseTaskForUser(insertTaskResult.ops[0]["_id"]);
                let taskDay = insertTaskResult.ops[0]["dayOfTheWeek"];
                let newValues = { $push: { [`tasks.${taskDay}`]: parsedTask } };
